...
function compile_atrios-bsp-cli-transitional() {
	...
	cat <<- EOF > "${destination}"/DEBIAN/control
		...
		Section: oldlibs
		Priority: optional
		Description: AtriOS CLI BSP for board '${BOARD}' - transitional package
	EOF
...
function compile_atrios-bsp-cli() {
	...
	cat <<- EOF > "${destination}"/DEBIAN/control
		...
		Description: AtriOS CLI BSP for board '${BOARD}' branch '${BRANCH}' ${extra_description[@]}
	EOF

	# atrioshwinfo, firstrun, atriosmonitor, etc. config file; also sourced in postinst
	mkdir -p "${destination}"/etc
	cat <<- EOF > "${destination}"/etc/atrios-release
	...
	mkdir -p "${destination}"/usr/share/atrios/
	...
	if [[ "${bootscript_info[has_bootscript]}" == "yes" ]]; then
		# Append some of it to atrios-release
		cat <<- EOF >> "${destination}"/etc/atrios-release
			BOOTSCRIPT_FORCE_UPDATE="${bootscript_info[bootscript_force_update]}"
			BOOTSCRIPT_DST="${bootscript_info[bootscript_dst]}"
		EOF

		# Using bootscript, copy it to /usr/share/atrios

		case "${bootscript_info[bootscript_src]}" in
			*'.template')
				display_alert "Rendering boot script template" "${bootscript_info[bootscript_file_fullpath]} -> ${destination}/usr/share/atrios/${bootscript_info[bootscript_dst]}" "info"
				run_host_command_logged cat "${bootscript_info[bootscript_file_fullpath]}" |
					render_bootscript_template > "${destination}/usr/share/atrios/${bootscript_info[bootscript_dst]}"

				if ! proof_rendered_bootscript_template "${destination}/usr/share/atrios/${bootscript_info[bootscript_dst]}"; then
					exit_with_error "Render of bootscript template was not successful. Inspect '${destination}/usr/share/atrios/${bootscript_info[bootscript_dst]}' for unrendered variables."
				fi
				;;
			*)
				display_alert "Deploying boot script" "${bootscript_info[bootscript_file_fullpath]} -> ${destination}/usr/share/atrios/${bootscript_info[bootscript_dst]}" "info"
				run_host_command_logged cp -pv "${bootscript_info[bootscript_file_fullpath]}" "${destination}/usr/share/atrios/${bootscript_info[bootscript_dst]}"
				;;
		esac

		if [[ "${bootscript_info[has_bootenv]}" == "yes" ]]; then
			run_host_command_logged cp -pv "${bootscript_info[bootenv_file_fullpath]}" "${destination}"/usr/share/atrios/atriosEnv.txt
		fi
	...
	# PRETTY_NAME stuff is now done in atrios-base-files artifact
	...
	# Keeping atrios-apt-updates as a configuration, solve the problem
	cat <<- EOF > "${destination}"/DEBIAN/conffiles
		/usr/lib/atrios/atrios-apt-updates
		/etc/X11/xorg.conf.d/01-atrios-defaults.conf
	EOF
	...
	for i in "${releases[@]}"; do
		echo "$(echo "$i" | sed 's/.*\///')=$(cat "$i"/support)$(echo ";upgrade" | sed 's/.*\///')=$(cat "$i"/upgrade)" >> "${destination}"/etc/atrios-distribution-status
	done
	...
	fingerprint_image "${destination}/etc/atrios.txt"
	...
	if [[ "${SHOW_DEBUG}" == "yes" ]]; then
		run_tool_batcat --file-name "/etc/atrios-release.sh" "${destination}"/etc/atrios-release
	fi
...
function reversion_atrios-bsp-cli_deb_contents() {
	...
	# Depends: fping is needed for atriosmonitor to upload atrios-hardware-monitor.log
	...
	if [[ "${SHOW_DEBUG}" == "yes" ]]; then
		run_tool_batcat --file-name "atrios-release.sh" "${data_dir}"/etc/atrios-release
	fi
...
function board_side_bsp_cli_postinst_update_uboot_bootscript() {
	if [[ ${BOOTSCRIPT_FORCE_UPDATE} == yes || ! -f /boot/${BOOTSCRIPT_DST} ]]; then
		...
		if [[ -f "/boot/${BOOTSCRIPT_DST}" ]]; then
			cp -v /boot/${BOOTSCRIPT_DST} /usr/share/atrios/${BOOTSCRIPT_DST}-${BOOTSCRIPT_BACKUP_VERSION}
			echo "NOTE: You can find previous bootscript versions in /usr/share/atrios !"
		fi

		echo "Recreating boot script"
		cp -v /usr/share/atrios/${BOOTSCRIPT_DST} /boot
		rootdev=$(sed -e 's/^.*root=//' -e 's/ .*\$//' < /proc/cmdline)
		rootfstype=$(sed -e 's/^.*rootfstype=//' -e 's/ .*$//' < /proc/cmdline)

		# recreate atriosEnv.txt if it and extlinux does not exists
		if [[ ! -f /boot/atriosEnv.txt && ! -f /boot/extlinux/extlinux.conf ]]; then
			cp -v /usr/share/atrios/atriosEnv.txt /boot
			echo "rootdev="\$rootdev >> /boot/atriosEnv.txt
			echo "rootfstype="\$rootfstype >> /boot/atriosEnv.txt
		fi
...
function board_side_bsp_cli_preinst() {
	...
	[[ -f "/etc/init.d/atrioshwinfo" ]] && rm /etc/init.d/atrioshwinfo
	[[ -f "/etc/logrotate.d/atrioshwinfo" ]] && rm /etc/logrotate.d/atrioshwinfo
	[[ -f "/etc/init.d/firstrun" ]] && rm /etc/init.d/firstrun
	[[ -f "/etc/init.d/resize2fs" ]] && rm /etc/init.d/resize2fs
	[[ -f "/lib/systemd/system/atrios-firstrun-config.service" ]] && rm /lib/systemd/system/atrios-firstrun-config.service
	...
	[[ -f "/usr/lib/atrios/apt-updates" ]] && rm /usr/lib/atrios/apt-updates
	[[ -f "/usr/lib/atrios/atrios-firstrun-config.sh" ]] && rm /usr/lib/atrios/atrios-firstrun-config.sh
...
function board_side_bsp_cli_postrm() { # not run here
	if [[ remove == "$1" ]] || [[ abort-install == "$1" ]]; then
		systemctl disable atrios-hardware-monitor.service atrios-hardware-optimize.service > /dev/null 2>&1
		systemctl disable atrios-zram-config.service atrios-ramlog.service > /dev/null 2>&1
		systemctl disable atrios-live-patch.service > /dev/null 2>&1
	fi
}
...
function board_side_bsp_cli_postinst_base() {
	...
	if [[ -n "$(grep -w '^ENABLED=false' /etc/default/log2ram 2> /dev/null)" ]]; then
		sed -i "s/^ENABLED=.*/ENABLED=false/" /etc/default/atrios-ramlog
	fi
...
function board_side_bsp_cli_postinst_finish() {
	...
	if [[ ! -f "/etc/default/atrios-motd" ]]; then
		mv /etc/default/atrios-motd.dpkg-dist /etc/default/atrios-motd
	fi
	if [[ ! -f "/etc/default/atrios-ramlog" && -f /etc/default/atrios-ramlog.dpkg-dist ]]; then
		mv /etc/default/atrios-ramlog.dpkg-dist /etc/default/atrios-ramlog
	fi
	if [[ ! -f "/etc/default/atrios-zram-config" && -f /etc/default/atrios-zram-config.dpkg-dist ]]; then
		mv /etc/default/atrios-zram-config.dpkg-dist /etc/default/atrios-zram-config
	fi
	if [[ ! -f "/etc/default/atrios-firstrun" ]]; then
		mv /etc/default/atrios-firstrun.dpkg-dist /etc/default/atrios-firstrun
	fi

	# Reload services
	systemctl --no-reload enable atrios-hardware-monitor.service atrios-hardware-optimize.service atrios-zram-config.service atrios-led-state.service > /dev/null 2>&1
}
